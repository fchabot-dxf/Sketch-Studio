...
```
              (MODULAR SOURCE - PRIMARY WORKING FILES)
...
output/                              (STANDALONE EXPORT - GENERATED FILES)
...
- `index.html` and `src/*.js` are the authoritative versions
- All logic lives here first
- This is where AI agents should make edits

### 3. **Export Files are Derivatives**
- `output/sketch-studio-unified-v1.0.html` is a bundled, inlined version
